...
def main():
    print("Creating final ensemble predictions including all Random Forest models...")
    ...
    print(f"\nFinal ensemble predictions saved to {output_file}")
    ...
    # Create a weighted ensemble that gives more weight to Random Forest models
    ...
    weights = {
        ...
        'poly': 1.0,
        'tree': 1.0,
        'rf': 2.0  # Double weight for Random Forest models
    }
    
    # Load predictions with weights
    weighted_sums = {}
    weight_totals = {}
    ...
    for file_path in prediction_files:
        ...
        weight = 1.0
        if 'lasso' in file_path:
            weight = weights['lasso']
        elif 'ridge' in file_path:
            weight = weights['ridge']
        elif 'poly' in file_path:
            weight = weights['poly']
        elif 'tree' in file_path:
            weight = weights['tree']
        elif 'rf' in file_path:
            weight = weights['rf']
        
        for id_val, pred in file_predictions.items():
            if id_val not in weighted_sums:
                weighted_sums[id_val] = 0.0
                weight_totals[id_val] = 0.0
            
            weighted_sums[id_val] += pred * weight
            weight_totals[id_val] += weight
    
    # Calculate weighted average predictions
    for id_val, weighted_sum in weighted_sums.items():
        if weight_totals[id_val] > 0:
            weighted_predictions[id_val] = weighted_sum / weight_totals[id_val]
    
    # Save weighted ensemble predictions
    weighted_output_file = "weighted_ensemble_predictions.csv"
    with open(weighted_output_file, 'w', newline='') as file:
        import csv
        writer = csv.writer(file)
        writer.writerow(['Id', 'SalePrice'])
        
        for id_val in sorted(weighted_predictions.keys()):
            pred = weighted_predictions[id_val]
            writer.writerow([id_val, round(pred, 4)])
    
    print(f"\nWeighted ensemble predictions saved to {weighted_output_file}")
    print("Random Forest models were given double weight in this ensemble")
