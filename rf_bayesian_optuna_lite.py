...
def objective(trial, X_train, y_train):
    ...
    # Define hyperparameters to search with reduced range
    n_estimators = trial.suggest_int('n_estimators', 50, 150)  # Reduced upper bound
    max_depth = trial.suggest_int('max_depth', 8, 20)  # Narrower range
    min_samples_split = trial.suggest_int('min_samples_split', 2, 6)  # Narrower range
    ...
    # Create KFold cross-validator with fewer folds
    kfold = KFold(n_splits=2, shuffle=True, random_state=42)  # Reduced from 3 to 2 folds
...
def main():
    print("Performing Optuna Bayesian Optimization for Random Forest hyperparameter tuning...")
    ...
    # Create Optuna study for hyperparameter optimization using TPE (Tree-structured Parzen Estimator)
    # TPE is a Bayesian optimization algorithm
    print("\nStarting Optuna Bayesian Optimization (TPE) for hyperparameter tuning...")
    ...
    study = optuna.create_study(
        ...
        sampler=optuna.samplers.TPESampler(seed=42)  # Bayesian optimization with fixed seed
    )
    
    # Number of trials for Bayesian optimization - reduced
    n_trials = 15  # Reduced from 30 to 15
    ...
    output_file = f"rf_bayesian_optuna_lite_{best_params_str}_predictions.csv"
    ...
    if hasattr(best_rf, 'feature_importances_') and best_rf.feature_importances_ is not None:
        ...
        feature_importance_df.to_csv('rf_bayesian_feature_importances_lite.csv', index=False)
        print(f"Feature importances saved to rf_bayesian_feature_importances_lite.csv")
    ...
    # Save to CSV
    trials_df.to_csv('rf_bayesian_trials_lite.csv', index=False)
    print(f"\nSaved all trial results to rf_bayesian_trials_lite.csv")
    ...
    plt.ylabel('RMSE')
    plt.title('Optuna Bayesian Optimization History')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    
    # Save the plot
    plt.savefig('rf_bayesian_optimization_history_lite.png')
    print("Saved optimization history plot to rf_bayesian_optimization_history_lite.png")
    ...
    with open('rf_bayesian_execution_time_lite.txt', 'w') as f:
        f.write(f"Optuna Bayesian Optimization Execution Time\n")
        f.write(f"----------------------------------------\n")
    ...
    # Save parameter importances to CSV
    param_importance_df.to_csv('rf_bayesian_param_importance_lite.csv', index=False)
    print(f"Parameter importances saved to rf_bayesian_param_importance_lite.csv")
    
    # Plot parameter importances
    plt.figure(figsize=(10, 6))
    plt.bar(param_importance_df['Parameter'], param_importance_df['Importance'])
    plt.xlabel('Parameter')
    plt.ylabel('Importance')
    plt.title('Parameter Importance in Bayesian Optimization')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('rf_bayesian_param_importance_lite.png')
    print("Saved parameter importance plot to rf_bayesian_param_importance_lite.png")
    
    print("\nRandom Forest Optuna Bayesian Optimization hyperparameter tuning complete!")
