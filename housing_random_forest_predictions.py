...
def main():
    print("Generating Random Forest predictions using optimized transformed data...")
    ...
    for i, params in enumerate(rf_params):
        print(f"\nTraining Random Forest model {i+1}/{len(rf_params)}:")
        ...
        output_file = f"optimized_rf_{params['n_estimators']}trees_depth{params['max_depth'] if params['max_depth'] else 'None'}_predictions.csv"
    ...
    # Create stacked predictions including Random Forest models
    print("\nUpdating stacked predictions to include Random Forest models...")
    ...
    # Calculate average predictions including Random Forest models
    final_predictions = {}
    ...
    output_file = "optimized_stacked_with_rf_predictions.csv"
    with open(output_file, 'w', newline='') as file:
        import csv
        writer = csv.writer(file)
        writer.writerow(['Id', 'SalePrice'])
    ...
    print("\nRandom Forest prediction process complete!")
