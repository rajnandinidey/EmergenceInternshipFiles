...
def plot_coefficient_path(X, y, feature_names, alphas=None, n_alphas=100):
    """
    Generate coefficient path plot for Ridge regression
    ...
    for i, alpha in enumerate(alphas):
        ridge = Ridge(alpha=alpha, fit_intercept=True, max_iter=10000)
...
def plot_cv_mse_curve(X, y, alphas=None, n_alphas=100, cv=5):
    """
    Generate cross-validation MSE curve for Ridge regression
    ...
    plt.title('Ridge Cross-Validation MSE Curve')
    plt.legend()
    plt.grid(True, which="both", ls="-")
    plt.tight_layout()
    plt.savefig('ridge_cv_mse_curve.png')
    plt.close()
    
    return best_alpha, avg_val_mse[best_alpha_idx]

def main():
    print("Ridge Regression Visualization")
    
    # Load training data
    train_path = "train_housingprices.csv"
    
    # Use the same features as in the housing_stacked_predictions.py
    feature_cols = [
    ...
    # Define a wide range of alpha values
    alphas = np.logspace(-4, 4, 100)
    
    # Plot coefficient path
    print("\nGenerating coefficient path plot...")
    plot_coefficient_path(X, y, feature_names, alphas)
    print("Saved coefficient path plot to 'ridge_coefficient_path.png'")
    ...
    print("Saved CV MSE curve to 'ridge_cv_mse_curve.png'")
    ...
    ridge = Ridge(alpha=best_alpha, fit_intercept=True, max_iter=10000)
    ridge.fit(X_scaled, y)
    
    # Print top 10 most important features
    coef_importance = sorted(zip(feature_names, ridge.coef_), key=lambda x: abs(x[1]), reverse=True)
