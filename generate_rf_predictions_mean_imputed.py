...
def main():
    print("Generating Random Forest predictions using best hyperparameters from different optimization methods with mean imputation...")
    ...
    if using_optimized:
        ...
        y_train = train_df['SalePrice_log1p']
        X_test = test_df[model_features]
        
        print(f"Using optimized data with target 'SalePrice_log1p'")
    else:
        ...
        X_train = train_encoded[common_cols]
        y_train = train_encoded['SalePrice']
        X_test = test_encoded[common_cols]
        model_features = common_cols
        
        print(f"Using original data with target 'SalePrice'")
    ...
    print("Applying mean imputation for missing values...")
    imputer = SimpleImputer(strategy='mean')
    X_train_imputed = pd.DataFrame(imputer.fit_transform(X_train), columns=X_train.columns)
    X_test_imputed = pd.DataFrame(imputer.transform(X_test), columns=X_test.columns)
    
    print(f"Missing values before imputation in training data: {X_train.isna().sum().sum()}")
    print(f"Missing values after imputation in training data: {X_train_imputed.isna().sum().sum()}")
    print(f"Missing values before imputation in test data: {X_test.isna().sum().sum()}")
    print(f"Missing values after imputation in test data: {X_test_imputed.isna().sum().sum()}")
    ...
    for method, params in best_params.items():
        ...
        output_file = f"rf_{method}_mean_imputed_predictions.csv"
    ...
    print("\nAll Random Forest predictions with mean imputation generated successfully!")
